...
1. Explain how you got the HTML, with the correct data, displayed in the browser?
   > Obtained a reference to the proper HTMLElement based on it's ID "potteryList', then used the "innerHTML" property to replace the default HTML with the HTMLbuilt programmatically.
2. In the **PotteryList** module, when you iterate your pottery, you need to show the evidence of what the **weight** property's value is for the 2nd piece of pottery. Use [Loom](https://www.loom.com/) to record your browser window with the developer tools open and show those values.
   > ***in the interest of catching up with the class I am deferring this***
3. The **PotteryWheel** module has a single function named `makePottery`. Why doesn't that module have all of the other code in it?
   > The code is modularized to aid in understanding and management.  This module contains only code relevant to creating a pottery object, including establishing the unique id for each object.
4. The pottery shop has learned that there is a set of customers that are willing to buy cracked pottery at a discounted price of $2.50. That means that the cracked pottery should now be displayed in the catalog. Explain the changes that this new business strategy would cause to your algorithm.
   > The function toSellOfNotToSell in module PotteryCatalog.js is the only place code needs to be updated.  An else can be added to the initial check for cracks which wouuld set the price equal to $2.50 if the pottery is cracked.  In all cases the pottery would be pushed to the end of the potteryToSell array, so the line of code which does this should be moved below the if conditional statements, just above the return statement.
